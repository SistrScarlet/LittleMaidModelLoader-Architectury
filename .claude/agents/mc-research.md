...
- プロジェクトのソース: `common/src/main/java/net/sistr/littlemaidmodelloader/` 配下
...
- コードブロック (``` ```) はメソッド/フィールドのシグネチャのみ許可
- 挙動・ロジック・使用パターン・例はすべて自然言語で記述する
- デコンパイルソース・プロジェクトファイル・ライブラリのコードを転載・再構成しない
