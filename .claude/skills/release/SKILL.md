...
Minecraft mod (LMML) のリリース作業を実行する。
...
gh release create v{version} \
  --title "v{version}" \
  -n "{リリースノート}" \
  fabric/build/libs/LMML-1.20.1-{version}-Fabric.jar \
  forge/build/libs/LMML-1.20.1-{version}-Forge.jar
```
...
既存のフォルダ構造 `/Mods/LittleMaidModelLoader/{Fabric,Forge}/{mc_folder}/` にアップロードされる。
...
- JARファイル名は `LMML-{mc_version}-{version}-Fabric.jar` / `LMML-{mc_version}-{version}-Forge.jar`
