...
使い方:
  python3 .claude/scripts/dropbox-upload.py --version 8.2.0 [--mc-version 1.20.1]
...
DROPBOX_BASE = "/Mods/LittleMaidModelLoader"
...
def main():
    ...
    parser.add_argument("--version", required=True, help="mod バージョン (例: 8.2.0)")
    ...
    jars = [
        (f"fabric/build/libs/LMML-{mc_version}-{version}-Fabric.jar",
         f"{DROPBOX_BASE}/Fabric/{mc_folder}/LMML-{mc_version}-{version}-Fabric.jar"),
        (f"forge/build/libs/LMML-{mc_version}-{version}-Forge.jar",
         f"{DROPBOX_BASE}/Forge/{mc_folder}/LMML-{mc_version}-{version}-Forge.jar"),
    ]
