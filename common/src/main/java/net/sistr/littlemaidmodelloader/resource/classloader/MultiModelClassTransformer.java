...
/** 古いマルチモデルのロード用。 使用しているクラスを置換えて新しいものへ対応。 */
public class MultiModelClassTransformer {
    ...
    private static final Map<String, String> CODE_REPLACE_MAP =
            new Object2ObjectOpenHashMap<>() {
                {
                    ...
                    put(
                            "mmmlibx/lib/MMM_EntityCaps",
                            "net/sistr/littlemaidmodelloader/maidmodel/EntityCaps");
                    put(
                            "littleMaidMobX/EntityCaps",
                            "net/sistr/littlemaidmodelloader/maidmodel/EntityCaps");
                    put(
                            "net/blacklab/lmr/util/EntityCaps",
                            "net/sistr/littlemaidmodelloader/maidmodel/EntityCaps");
                    put(
                            "net/blacklab/lmr/util/EntityCapsLiving",
                            "net/sistr/littlemaidmodelloader/maidmodel/EntityCaps");
                    ...
                    put(
                            ...
                            "net/sistr/littlemaidmodelloader/entity/EntityLittleMaid");
                    put(
                    ...
                    // NM
                    put(
                            "net/blacklab/lmr/entity/littlemaid/EntityLittleMaid",
                            "net/sistr/littlemaidmodelloader/entity/EntityLittleMaid");
                ...
                private void addTransformTarget(String className) {
                    ...
                    put(
                            "mmmlibx/lib/multiModel/model/mc162/" + className,
                            PACKAGE_STRING + className);
                    put(
                            "net/blacklab/lmr/entity/maidmodel/" + className,
                            PACKAGE_STRING + className);
                }
    ...
    private static final Set<String> GL_REPLACE_MODEL_RENDERER_SET =
    ...
    private static final Set<String> GL_REPLACE_DUMMY_SET =
            new ObjectOpenHashSet<>() {
                {
                    add("()V");
                    add("(I)V");
                ...
                .forEach(
                        fNode -> {
                            ...
                            tryReplace(
                                    changed,
                                    fNode.signature,
                                    signature -> fNode.signature = signature);
                        });

        // メソッドの置き換え
        cNode.methods.parallelStream()
                .forEach(
                        mNode -> {
                            tryReplace(changed, mNode.desc, desc -> mNode.desc = desc);

                            if (mNode.localVariables != null) {
                                mNode.localVariables.parallelStream()
                                        .forEach(
                                                lNode -> {
                                                    if (lNode.desc != null) {
                                                        tryReplace(
                                                                changed,
                                                                lNode.desc,
                                                                desc -> lNode.desc = desc);
                                                    }
                                                    if (lNode.name != null) {
                                                        tryReplace(
                                                                changed,
                                                                lNode.name,
                                                                name -> lNode.name = name);
                                                    }
                            ...
                            while (aNode != null) {
                                if (aNode instanceof FieldInsnNode fANode) { // 4
                                    ...
                                    tryReplace(
                                            changed, fANode.owner, owner -> fANode.owner = owner);
                                    ...
                                    if (shouldRemove(fANode.owner)) {
                                        ...
                                        aNode = aNode.getNext();
                                        // 置き換え対象があるならそちらへ
                                        if (GL_REPLACE_MODEL_RENDERER_SET.contains(
                                                fANode.name + fANode.desc)) {
                                            mNode.instructions.set(
                                                    fANode,
                                                    new MethodInsnNode(
                                                            fANode.getOpcode(),
                                    ...
                                    tryReplace(changed, fANode.desc, desc -> fANode.desc = desc);
                                } else if (aNode instanceof LdcInsnNode fANode
                                        && ((LdcInsnNode) aNode).cst instanceof Type) {
                                    tryReplace(
                                            changed,
                                            ((Type) fANode.cst).getInternalName(),
    ...
    private void tryReplace(AtomicBoolean changed, String text, Consumer<String> replacer) {
        ...
        for (Entry<String, String> entry : CODE_REPLACE_MAP.entrySet()) {
            if ((text + ";").contains(entry.getKey() + ";")) {
                ...
                newText = text;
                // 量がエグいので基本表示しないように
                // if (LMMLConfig.isDebugMode()) LOGGER.debug("Replacing : " + text + " to " +
                // newText);
