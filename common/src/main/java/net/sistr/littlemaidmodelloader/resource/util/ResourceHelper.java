...
public class ResourceHelper {
    ...
    /** xxxx/yyyy/[texture]_[model] のうち、[model]を抜き取る */
    ...
    /**
     * assets/minecraft/textures/entity/littlemaid/[texture]_[model]/xxxx_[index].png
     * のうち、[index]を抜き取って16進数変換 [index]が存在しない場合、-1を返す
     */
    ...
    public static Optional<String> getFirstParentName(
            String path, Path homePath, boolean isArchive) {
        if (isArchive) {
            // zipファイル名を取る
    ...
    /** ResourceWrapperに登録するファイル名を取得する。 prefixはResourceWrapperでの読み込み時に使う */
