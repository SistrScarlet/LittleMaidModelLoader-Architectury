...
public class ArmorPart {
  ...
  public ArmorPart(
      Identifier innerTex,
      Identifier innerTexLight,
      Identifier outerTex,
      Identifier outerTexLight,
  ...
  public static final class Builder {
    private static final Map<TextureHolder, List<WeakReference<ArmorPart>>> REFERENCES =
        new HashMap<>();
    ...
    public ArmorPart build() {
      ...
      // getNewDataAndCache(textureHolder, innerTex, innerTexLight, outerTex, outerTexLight,
      // innerModel, outerModel);
    }

    // 参照されてる=他のメイドが使用してる防具パーツモデルは使いまわす
    // メモリを削減できたとしても負荷的にはどうなのか…早すぎる最適化な気もする
    private static ArmorPart getNewDataAndCache(
      ...
      ArmorPart data =
          new ArmorPart(innerTex, innerTexLight, outerTex, outerTexLight, innerModel, outerModel);
