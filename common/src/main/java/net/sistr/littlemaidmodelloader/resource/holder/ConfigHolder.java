...
public class ConfigHolder {
    ...
    public Optional<String> getSoundFileName(String soundName) {
        ...
        return optional.map(ResourceHelper::removeNameLastIndex) // 複数ファイルの数字ではなく、素で数字が末尾に付いてるファイル対策
                .map(
                        fileName -> {
                            // [親].[ファイル]に変換
                            ...
                            int secondLastSplitter =
                                    fileName.substring(0, lastSplitter).lastIndexOf(".");
