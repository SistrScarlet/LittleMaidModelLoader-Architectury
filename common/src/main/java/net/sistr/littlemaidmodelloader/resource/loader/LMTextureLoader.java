...
/** 画像ファイルを読み込み、ゲームに登録するクラス メイドさんのランダムテクスチャのため、サーバーでもテクスチャ名だけは読む */
public class LMTextureLoader implements LMLoader {
  ...
  public void load(String path, Path folderPath, InputStream inputStream, boolean isArchive) {
    Identifier texturePath =
        getResourceLocation(path, isArchive)
            .orElseThrow(() -> new IllegalArgumentException("引数が不正です。"));
    String textureName =
        ResourceHelper.getTexturePackName(path, isArchive)
            .orElseThrow(() -> new IllegalArgumentException("引数が不正です。"));
    String modelName = ResourceHelper.getModelName(textureName);
    textureManager.addTexture(
        ResourceHelper.getFileName(path, isArchive),
        textureName,
        modelName,
        ResourceHelper.getIndex(path),
        texturePath);
  ...
  /**
   * パスをテクスチャのパスに変換 assets/minecraft/textures/entity/littlemaid/[texture]_[model]/xxxx_[index].png
   * または mob/littlemaid/[texture]_[model]/xxxx_[index].png を
   * littlemaidmodelloader:textures.entity.littlemaid.[texture]_[model].xxxx_[index].png に変換
   */
