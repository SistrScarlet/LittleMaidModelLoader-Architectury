...
public class TextureHolder {
    ...
    public void addArmorTexture(String armorType, int index, Identifier texturePath) {
        Map<Integer, Identifier> armorMap =
                armors.computeIfAbsent(armorType.toLowerCase(), k -> new HashMap<>());
        armorMap.put(index, texturePath);
    }

    public Optional<Identifier> getTexture(
            TextureColors color, boolean isContract, boolean isLight) {
        int index = color.getIndex();
        if (isLight) {
            index +=
                    isContract
                            ? TextureIndexes.COLOR_CONTRACT_LIGHT.getIndexMin()
                            : TextureIndexes.COLOR_WILD_LIGHT.getIndexMin();
        } else if (!isContract) {
    ...
    public Optional<Identifier> getArmorTexture(
            IHasMultiModel.Layer layer, String armorName, float damagePercent, boolean isLight) {
        if (armors.isEmpty()) {
            return Optional.empty();
        }
        Optional<Identifier> optional =
                getArmorTextureInner(layer, armorName, damagePercent, isLight);
    ...
    public Optional<Identifier> getArmorTextureInner(
            IHasMultiModel.Layer layer, String armorName, float damagePercent, boolean isLight) {
        // 返すものが無いなら当然空
        if (armors.isEmpty()) {
            return Optional.empty();
        ...
        switch (layer) {
            case INNER:
                index =
                        (isLight
                                        ? TextureIndexes.ARMOR_1_DAMAGED_LIGHT
                                        : TextureIndexes.ARMOR_1_DAMAGED)
                                .getIndexMin();
                break;
            case OUTER:
                index =
                        (isLight
                                        ? TextureIndexes.ARMOR_2_DAMAGED_LIGHT
                                        : TextureIndexes.ARMOR_2_DAMAGED)
                                .getIndexMin();
                break;
            default:
    ...
    public boolean hasSkinTexture(boolean isContract) {
        for (TextureColors color : TextureColors.values()) {
            if (getTexture(color, isContract, false).isPresent()
                    || getTexture(color, isContract, true).isPresent()) return true;
        }
        return false;
    }
