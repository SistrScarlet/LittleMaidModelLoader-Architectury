...
public class MultiModelCompound implements IHasMultiModel {
    ...
    public MultiModelCompound(
            LivingEntity entity,
            TextureHolder defaultMainPackage,
            TextureHolder defaultArmorPackage) {
        this.entity = entity;
    ...
    public void updateMain() {
        ...
        if (skinModel instanceof IModelCaps modelCaps) {
            modelCaps.setCapsValue(IModelCaps.caps_changeModel, caps);
        }
        skinTexture =
                new TexturePair(
                        skinTexHolder.getTexture(color, isContract, false).orElse(null),
    ...
    public void updateArmor() {
        ...
        for (ItemStack stack : entity.getArmorItems()) {
            ...
            updateArmorPart(
                    Part.getPart(index++), getName(stack.getItem()), getDamagePercent(stack));
        }
    }
    ...
    private void updateArmorPart(Part part, String armorName, float damagePercent) {
        ...
        dataBuilder.innerModel(
                manager.getOrDefaultModel(textureHolder.getModelName(), Layer.INNER));
        dataBuilder.outerModel(
                manager.getOrDefaultModel(textureHolder.getModelName(), Layer.OUTER));
        dataBuilder.innerTex(
                textureHolder
                        .getArmorTexture(Layer.INNER, armorName, damagePercent, false)
                        .orElse(null));
        dataBuilder.innerTexLight(
                textureHolder
                        .getArmorTexture(Layer.INNER, armorName, damagePercent, true)
                        .orElse(null));
        dataBuilder.outerTex(
                textureHolder
                        .getArmorTexture(Layer.OUTER, armorName, damagePercent, false)
                        .orElse(null));
        dataBuilder.outerTexLight(
                textureHolder
                        .getArmorTexture(Layer.OUTER, armorName, damagePercent, true)
                        .orElse(null));
    ...
    public void writeToNbt(NbtCompound nbt) {
        ...
        for (Part part : Part.values()) {
            nbt.putString(
                    "ArmorTextureInner" + part.getPartName(),
                    getTextureHolder(Layer.INNER, part).getTextureName());
            nbt.putString(
                    "ArmorTextureOuter" + part.getPartName(),
    ...
    public void readFromNbt(NbtCompound nbt) {
        ...
        if (nbt.contains("SkinTexture")) {
            textureManager
                    .getTexture(nbt.getString("SkinTexture"))
                    .ifPresent(
                            textureHolder ->
                                    setTextureHolder(textureHolder, Layer.SKIN, Part.HEAD));
        }
        for (Part part : Part.values()) {
            ...
            if (nbt.contains(inner)) {
                textureManager
                        .getTexture(nbt.getString(inner))
                        .ifPresent(
                                textureHolder ->
                                        setTextureHolder(textureHolder, Layer.INNER, part));
            }
            if (nbt.contains(outer)) {
                textureManager
                        .getTexture(nbt.getString(outer))
                        .ifPresent(
                                textureHolder ->
                                        setTextureHolder(textureHolder, Layer.OUTER, part));
            }
        }
    }
