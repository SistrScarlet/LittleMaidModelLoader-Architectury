...
/**
 * マルチモデル用の基本クラス、これを継承していればマルチモデルとして使用できる。 Mincraftネイティブなクラスや継承関数などを排除して、難読化対策を行う。
...
public abstract class ModelMultiBase extends ModelBase implements IModelCaps, IMultiModel {
    ...
    public ModelMultiBase(
            float pSizeAdjust, float pYOffset, int pTextureWidth, int pTextureHeight) {
        isSneak = false;
        aimedBow = false;
    ...
    public void setupTransform(
        ...
        if (ModelCapsHelper.getCapsValueBoolean(caps, IModelCaps.caps_isFallFlying)) {
                        ...
                        k
                                * (-90.0F
                                        - ModelCapsHelper.getCapsValueFloat(
                                                caps, IModelCaps.caps_rotationPitch)));
            }
            ...
            roll =
                    ModelCapsHelper.getCapsValueBoolean(caps, IModelCaps.caps_isInWater)
                            ? -90.0F
                                    - ModelCapsHelper.getCapsValueFloat(
                                            caps, IModelCaps.caps_rotationPitch)
                            : -90.0F;
    ...
    private Vec3d getRotationVec(IModelCaps caps, float tickDelta) {
        ...
        float prevPitch =
                ModelCapsHelper.getCapsValueFloat(caps, IModelCaps.caps_prevRotationPitch);
        return getRotationVector(
                MathHelper.lerp(tickDelta, prevPitch, pitch),
                MathHelper.lerp(tickDelta, prevYaw, yaw));
    }
    ...
    public void animateModel(
            IModelCaps caps, float limbAngle, float limbDistance, float tickDelta) {
    ...
    public void setAngles(
        ...
        setRotationAngles(
                limbAngle, limbDistance, animationProgress, headYaw, headPitch, 0.0625F, caps);
    }
    ...
    /**
     * モデル指定詞に依らずに使用するテクスチャパック名。 一つのテクスチャに複数のモデルを割り当てる時に使う。
     *
     * @return
     */
     ...
     * 部位ごとの装甲表示。
     *
     * @param parts 3:頭部。 2:胴部。 1:脚部 0:足部
     * @param index 0:inner 1:outer
     * @return 戻り値は基本 -1
     */
