...
public class ModelLittleMaid_RX0 extends ModelLittleMaidBase {
    ...
    public void setDefaultPause(
        ...
        if ((!ModelCapsHelper.getCapsValueBoolean(pEntityCaps, caps_isOverdriveDelay)
            ...
            bipedRibbon.setRotateAngleDegX(
                    20F - bipedHead.getRotateAngleDegX() + (isSneak ? 20F : 0F));
            ...
            bipedRibbonR
                    .setRotationPoint(-3.5F, 0F, 4F)
                    .setRotateAngleDeg(
                            15F - bipedHead.getRotateAngleDegX() + (isSneak ? 10F : 0F), 0F, 10F);
    ...
    public float getHeight(IModelCaps pEntityCaps) {
        int lvisible = ModelCapsHelper.getCapsValueInt(pEntityCaps, caps_PartsVisible);
        boolean lf =
                (!ModelCapsHelper.getCapsValueBoolean(pEntityCaps, caps_isOverdriveDelay)
                                && !ModelCapsHelper.getCapsValueBoolean(
                                        pEntityCaps, caps_isBloodsuck)
                                && ((lvisible & 0x0001) == 0))
                        && true;
