...
/** GL互換クラス 完全ではないが、メイドモデル読み込みに限っては概ね問題無い 問題が出るモデルが出るたびに更新すること */
public final class GLCompat {
  ...
  public static void glRotatef(float deg, float x, float y, float z) {
    if (mode == GL11.GL_MODELVIEW) {
      ModelRenderer.matrixStack.multiply(
          RotationAxis.of(new Vector3f(x, y, z)).rotationDegrees(deg));
    } else if (mode == GL11.GL_TEXTURE) {
  ...
  private static void combine() {
    if (tex != null && pos != null) {
      ...
      tex = null;
      if (vertexPrev2 != null) {
        ModelBoxBase.TexturedQuad quad =
            new ModelBoxBase.TexturedQuad(
                new ModelBoxBase.PositionTextureVertex[] {
                  vertexPrev2, vertexPrev1, vertexCurrent, vertexCurrent
                });
        quad.draw(
            ModelRenderer.matrixStack,
            ModelRenderer.buffer,
