...
public class ModelLittleMaid_Elsa5 extends ModelLittleMaidBase {
    ...
    public void initModel(float psize, float pyoffset) {
        ...
        bipedBody
                .setTextureOffset(20, 20)
                .addBox(-3F, -4.5F, -2.21F, 6, 2, 2, psize + 0.2F); // breast
        ...
        blinkFreq +=
                1F
                        - (float) ModelCapsHelper.getCapsValueInt(pEntityCaps, caps_health)
                                / 20F; // �̗͏��Ȃ��Ƃ܂΂��������Ȃ�
        float f3 =
                (float) (entityTicksExisted + pRenderPartialTicks + entityIdFactor) * 0.01F; // �ʑ�
        float f4 =
                (float)
                        (Math.sin(f3 * 3F)
                                + Math.sin(f3 * 17F)
    ...
    public void setRotationAngles(
        ...
        Arms[2].setRotateAngle(
                -0.78539816339744830961566084581988F - bipedRightArm.getRotateAngleX(), 0F, 0F);
        Arms[3].setRotateAngle(
                -0.78539816339744830961566084581988F - bipedLeftArm.getRotateAngleX(), 0F, 0F);
    }
}
