...
public class ModelSelectScreen<T extends Entity & IHasMultiModel> extends Screen {
    ...
    protected void init() {
        ...
        textureHolders.forEach(
                textureHolder ->
                        map.put(textureHolder.getTextureName().toLowerCase(), textureHolder));
        initModelGUI(textureHolders, map);
        initArmorGUI(textureHolders, map);
    }

    protected void initModelGUI(
            Collection<TextureHolder> textureHolders, Map<String, TextureHolder> textureHolderMap) {
        int allColor = 16;
                ...
                FilterableListGUI.<MultiModelGUI>builder()
                        ...
                        .items(
                                textureHolders.stream()
                                        ...
                                        .filter(
                                                textureHolder ->
                                                        textureHolder.hasSkinTexture(
                                                                        this.isContract)
                                                                && modelManager
                                                                        .getModel(
                                                                                textureHolder
                                                                                        .getModelName(),
                                                                                IHasMultiModel.Layer
                                                                                        .SKIN)
                                                                        .isPresent())
                                        .map(
                                                t ->
    ...
    protected void initArmorGUI(
            Collection<TextureHolder> textureHolders, Map<String, TextureHolder> map) {
        LMModelManager modelManager = LMModelManager.INSTANCE;
        int allColor = 16;
        ...
        FilterPredicate<ArmorModelGUI> armorModelFilter =
                (armorModelGUI, filterText) -> {
                    String textureName = armorModelGUI.getTexture().getTextureName().toLowerCase();
                    return textureName.contains(filterText.toLowerCase());
                };

        this.armorListGUI =
                FilterableListGUI.<ArmorModelGUI>builder()
                        .position((width - listWidth) / 2, (height - listHeight) / 2)
                        .size(listWidth, listHeight)
                        .elementSize(listWidth, scale * heightRatio)
                        .items(
                                textureHolders.stream()
                                        ...
                                        .map(
                                                t ->
                                                        new ArmorModelGUI(
                                                                t, scale, this.dummy, this.armors))
                                        .collect(Collectors.toList()))
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float partialTicks) {
        ...
        context.drawTexture(
                MODEL_SELECT_GUI_TEXTURE, relX - 24, relY + GUI_HEIGHT - 16, 0, 240, 16, 16);
        context.drawTexture(
                MODEL_SELECT_GUI_TEXTURE, relX - 24, relY + GUI_HEIGHT - 48, 0, 240, 16, 16);
                                    ...
                                    g.getSelectColor()
                                            .ifPresent(
                                                    color -> {
                                                        ...
                                                        MultiModelGUIUtil.getModel(
                                                                        LMModelManager.INSTANCE,
                                                                        texture)
                                                                .ifPresent(
                                                                        model -> {
                                                                            int scale = 15 * 3;
                        ...
                        MultiModelGUIUtil.getModel(LMModelManager.INSTANCE, texture)
                                .ifPresent(
                                        model -> {
                                            ...
                                            LMModelManager modelManager = LMModelManager.INSTANCE;
                                            ArmorPart armorData =
                                                    MultiModelGUIUtil.getArmorDate(
                                                            modelManager, texture, "default");
    ...
    public void removed() {
                                        ...
                                        .ifPresent(
                                                color -> {
                                                    ...
                                                    entity.setTextureHolder(
                                                            texture,
                                                            IHasMultiModel.Layer.SKIN,
                                                            IHasMultiModel.Part.HEAD);
                                                    // 防具をスキンと同様に更新
                                                    for (IHasMultiModel.Part part :
                                                            IHasMultiModel.Part.values()) {
                                                        entity.setTextureHolder(
                                                                texture,
        ...
        this.armors.foreach(
                (p, g) -> entity.setTextureHolder(g.getTexture(), IHasMultiModel.Layer.INNER, p));

        ArmorSets<String> armorNames = new ArmorSets<>();
        for (IHasMultiModel.Part part : IHasMultiModel.Part.values()) {
            armorNames.setArmor(
                    entity.getTextureHolder(IHasMultiModel.Layer.INNER, part).getTextureName(),
                    part);
        }
