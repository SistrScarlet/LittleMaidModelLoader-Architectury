...
public class MultiModelGUIUtil {
  ...
  public static Optional<TexturePair> getTexturePair(
    ...
    return optional.map(
        resourceLocation ->
            new TexturePair(
                resourceLocation, holder.getTexture(color, isContract, true).orElse(null)));
  }
  ...
  public static ArmorPart getArmorDate(
    ...
    IMultiModel outerModel =
        modelManager
            ...
            .orElseThrow(() -> new IllegalStateException("モデルが存在しません"));
    Identifier innerTex =
        texture.getArmorTexture(IHasMultiModel.Layer.INNER, armorName, 0, false).orElse(null);
    Identifier innerLightTex =
        texture.getArmorTexture(IHasMultiModel.Layer.INNER, armorName, 0, true).orElse(null);
    Identifier outerTex =
        texture.getArmorTexture(IHasMultiModel.Layer.OUTER, armorName, 0, false).orElse(null);
    Identifier outerLightTex =
        texture.getArmorTexture(IHasMultiModel.Layer.OUTER, armorName, 0, true).orElse(null);
