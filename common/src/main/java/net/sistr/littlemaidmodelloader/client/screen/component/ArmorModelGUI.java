...
public class ArmorModelGUI extends GUIElement implements ListGUIElement {
  ...
  public void render(DrawContext context, int mouseX, int mouseY, float delta) {
    MultiModelGUIUtil.getModel(LMModelManager.INSTANCE, texture)
        .ifPresent(
            model -> renderAllArmorModel(context, scale, mouseX, mouseY, model, texture, dummy));
  }

  public void renderAllArmorModel(
      DrawContext context,
      int scale,
      float mouseX,
      float mouseY,
  ...
  public boolean mouseReleased(double mouseX, double mouseY, int button) {
    if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
      if (selectBox.release(mouseX, mouseY)) {
        ...
        if (this.width - 16 * 4 <= relativeX
            && relativeX < this.width
            && fontRenderer.fontHeight <= relativeY
            && relativeY < fontRenderer.fontHeight + 16) {
